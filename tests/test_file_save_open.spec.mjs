...
test('Test markdown export with text colors', async ({ page, browserName }) => {
  test.skip(browserName === "chromium", "Playwright does not currently support the File System Access API");

  await page.goto('/');
  await page.locator('text=Accept').click();

  const path = "tests/test_md_export_text_colors.epxyz";
  page.once('filechooser', async (fileChooser) => {
    await fileChooser.setFiles(path);
  });
  ...
  const reference_content = await fs.readFile('./tests/test_md_export_text_colors_reference.md', 'utf8');
  const exported_content = await fs.readFile(mdPath, 'utf8');
