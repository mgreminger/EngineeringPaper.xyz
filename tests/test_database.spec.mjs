...
test('Test database', async ({ page, browserName }) => {
  ...
  await page.type(':nth-match(math-field._editable, 2)', 'cos(x)=');
  ...
  await page.type(':nth-match(math-field._editable, 1)', 'y=x');
  await page.click('#add-math-cell');
  await page.type(':nth-match(math-field._editable, 2)', 'sigma=-x');
  await page.click('#add-math-cell');
  await page.type(':nth-match(math-field._editable, 3)', 'y(-1<=x<=1)=');
  await page.locator('#add-row-3').click();
  await page.type(':nth-match(math-field._editable, 4)', 'sigma(-1<x<1)=');
  ...
  await page.locator('#plot-expression-5-1 math-field._editable').type('sigma(-1[inch]<=x<=1[inch])=[m]');
