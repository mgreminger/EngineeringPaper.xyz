...
test('Test imaginary number unit conversions', async ({ page }) => {
  ...
  await page.waitForSelector('text=Updating...', {state: 'detached'});
  await page.locator('#cell-3 >> text=Dimension Error').waitFor({state: 'attached', timeout: 100});
  ...
  await page.waitForSelector('text=Updating...', {state: 'detached'});
  await page.locator('#cell-3 >> text=Units Mismatch').waitFor({state: 'attached', timeout: 100});
...
test('Test angle function', async ({ page }) => {
  ...
  await page.locator('#cell-2 >> text=Dimension Error').waitFor({state: "attached", timeout: 100});
