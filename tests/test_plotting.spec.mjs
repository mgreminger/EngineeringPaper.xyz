...
test('Test plotting', async ({ browserName }) => {
  ...
  await page.type(':nth-match(math-field, 2)', 'z=-x');
  await page.click('#add-math-cell');
  await page.type(':nth-match(math-field, 3)', 'y(-1<=x<=1)=');
  await page.locator('#plot-expression-3-0 math-field').press('Enter');
  await page.type(':nth-match(math-field, 4)', 'z(-1<x<1)=');
  ...
  await page.locator('#plot-expression-5-0 math-field').press('Enter');
  await page.locator('#plot-expression-5-1 math-field').type('y(-1<=s<=1)=');
  await page.waitForSelector('button:has-text("Results of expression does not evaluate to finite and real numeric values")');
  for (let i = 0; i < 12; i++) {
    await page.locator('#plot-expression-5-1 math-field').press('Backspace');
  }

  await page.locator('#plot-expression-5-1 math-field').type('y(-1<=x<=1)=');
  await page.pause();
  ...
  await page.locator('#plot-expression-5-1 math-field').type('y=');
  ...
  await page.locator('#plot-expression-5-1 math-field').type('y(-s<x<s)=');
  await page.waitForSelector('button:has-text("X-axis limits of plot do not evaluate to a number")');
  for (let i = 0; i < 10; i++) {
    await page.locator('#plot-expression-5-1 math-field').press('Backspace');
  }

  await page.locator('#plot-expression-5-1 math-field').type('y(1[inch]<x<1[sec])=');
  await page.waitForSelector('button:has-text("Units of the x-axis upper and lower limits do not match")');
  for (let i = 0; i < 20; i++) {
    await page.locator('#plot-expression-5-1 math-field').press('Backspace');
  }

  await page.locator('#plot-expression-5-1 math-field').type('y(-1[inch]<x<1[inch])=[sec]');
  await page.waitForSelector('button:has-text("Units Mismatch")');
  for (let i = 0; i < 29; i++) {
    await page.locator('#plot-expression-5-1 math-field').press('Backspace');
  }

  await page.locator('#plot-expression-5-1 math-field').type('y(-1<x<1, -1<s<1)=');
  ...
  await page.waitForSelector('button:has-text("Results of expression does not evaluate to finite and real numeric values")');
  for (let i = 0; i < 18; i++) {
    await page.locator('#plot-expression-7-0 math-field').press('Backspace');
  }

  await page.locator('#plot-expression-7-0 math-field').type('s(v=p,0<u<20)=');
  await page.waitForSelector('button:has-text("Results of expression does not evaluate to finite and real numeric values")');
  for (let i = 0; i < 14; i++) {
    await page.locator('#plot-expression-7-0 math-field').press('Backspace');
  }

  await page.locator('#plot-expression-7-0 math-field').type('s(v=pi/4');
  await page.locator('#plot-expression-7-0 math-field').press('ArrowRight');
  await page.locator('#plot-expression-7-0 math-field').type(',0<u<20)=');
});
...
test('Test plot two curves with compatible x-range units', async ({ browserName }) => {
  ...
  await page.locator('math-field').nth(2).type('y(0[m]<=x<=10[m])=[m]');
...
test('Test plot number of points', async ({ browserName }) => {
  ...
  await page.click('#add-math-cell');
  await page.locator('math-field').nth(2).type('y(0<=x<=1)=');
  ...
  for (let i = 0; i < 40; i++) {
    await page.locator('math-field').nth(2).press('Backspace');
  }
  await page.locator('math-field').nth(2).type('z(0<=s<=1) with 1 points =');
  page.locator('button:has-text("Number of range points must be 2 or greater.")').waitFor({timeout: 1000 })

  for (let i = 0; i < 40; i++) {
    await page.locator('math-field').nth(2).press('Backspace');
  }
  await page.locator('math-field').nth(2).type('z(s=1) with 10 points =');
  ...
  await page.locator('math-field').nth(2).type('y(x=z(0<=s<=1)with 10 points)=');
  ...
  await page.locator('math-field').nth(2).type('y(0<=x<=1) with 2 points =');
...
test('Test copy plot data', async ({ browserName }) => {
  ...
  await page.locator('#add-plot-cell').click();
  await page.locator('#plot-expression-2-0 math-field').type('y1(-10[inch]<=x<=10[inch])with 2 points=');
  await page.locator('#add-row-2').click();
  await page.locator('#plot-expression-2-1 math-field').type('y2(10[inch]<=x<=20[inch])with 2 points=');
