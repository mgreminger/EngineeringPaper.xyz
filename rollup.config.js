...
export default [
  {
    ...
    plugins: [
      del({
        targets: ["public/_worker.js", "public/serviceworker.*"],
        runOnce: true,
      }),
  ...
  {
    input: "src/jediWorker.ts",
    output: {
      format: "iife",
      file: "public/jediWorker.js",
    },
    plugins: [
	  optimizeImports(),
	  commonjs(),
    ...
    plugins: [
      ...
      generateSW(
        ...
        function render({ swDest, count, size }) {
          console.log(
            `Service worker ${swDest} set to precache ${count} files totalling ${
              size / 1000 ** 2
            } MB.`
          );
        }
