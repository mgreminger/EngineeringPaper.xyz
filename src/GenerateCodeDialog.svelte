...
${statement.parameterNames.map(parameterMap).join("\n")}
    ...
    ${isMatrixResult(result) ? "numpy.array" : "float"}
        Return value ${isMatrixResult(result) ? getMatrixUnitsResult(result, statement.units) : getUnitsDescription(statement.units ?? result.units)}
    """
